...
# This script should be executed from within your fork
#
# The script will...
# 1. automatically create remote branches for upstream and the openshift fork
# 2. create a new branch using the latest openshift fork's main
...
UPSTREAM_REMOTE_REPO="https://github.com/kubevirt/cloud-provider-kubevirt.git"
DOWNSTREAM_REMOTE_REPO="https://github.com/openshift/cloud-provider-kubevirt.git"

DOWNSTREAM_GH_REPO="openshift/cluster-api-provider-kubevirt"
...
git remote add $OPENSHIFT_REMOTE $DOWNSTREAM_REMOTE_REPO
set -e
...
gh pr create -B main -t "auto sync ${TODAY}" -b "auto sync upstream ${TODAY}" -R $DOWNSTREAM_GH_REPO 
